...
- `/appointments` — «Записи»: **being built again, and this is the third attempt.** Two earlier designs were each taken down whole rather than edited, so this one inherits nothing from either — v1 (a 24-hour time grid) is in commit `1e0c045`, v2 (a full-width month calendar) was archived under `src/archive/` and that folder has since been deleted; both are in git history.
  ...
  The shape this time is **a picker beside a day**: `components/appointments/MonthCalendar.jsx` sits top-left and chooses a date, and the column next to it will show what is booked on it. That column is **genuinely empty**, not a placeholder — the calendar is the first piece and the rest is being built onto it.
