...
- `/appointments` — «Записи»: **being built again, and this is the third attempt.** Two earlier designs were each taken down whole rather than edited, so this one inherits nothing from either — v1 (a 24-hour time grid) is in commit `1e0c045`, v2 (a full-width month calendar) was archived under `src/archive/` and that folder has since been deleted; both are in git history.
  ...
  **The timetable is not a card, and the row of cards above it is.** It has no radius and no fill — only rules above and below, and `-mx-4` on its wrapper cancels the page padding so it runs from the rail to the window edge. Its top rule then begins exactly where the rail's vertical line ends, which is what makes it read as part of the shell rather than as one more block lying on the page. The consequence to remember: with no fill of its own, the sticky column headings take **`bg-ground`**, not `bg-surface`.
