...
- `/appointments` — «Записи»: **being built again, and this is the third attempt.** Two earlier designs were each taken down whole rather than edited, so this one inherits nothing from either — v1 (a 24-hour time grid) is in commit `1e0c045`, v2 (a full-width month calendar) was archived under `src/archive/` and that folder has since been deleted; both are in git history.

  The shape this time is **a row of 300px cards over a week timetable**. `components/appointments/MonthCalendar.jsx` sits top-left and picks a day; two cards beside it are still empty surfaces, deliberately bare — a heading would be a claim about what goes in them and that is not decided. Below them `components/appointments/WeekTimetable.jsx` draws the week that day falls in.

  The row is a **wrapping flex line of fixed 300px cards**, not named grid columns: the browser fits as many per line as the viewport allows and moves the rest down, which is the same answer at every width without a breakpoint doing arithmetic about how many cards clear 1024px. Heights match for free — flex items stretch to their row — and the `min-h-[302px]` on the empty two is the floor for when they are alone on a line. That number is the calendar's own height and it is arithmetic, not a guess: 32 padding + 32 header + 12 gap + 24 weekday row + 202 of grid.

  The page is a **flex row on the page element itself**, which is load-bearing: `items-stretch` measures against the CSS module's `min-height`, and that is the only way the right-hand panel is genuinely full height. Wrapped in a plain div it would be as tall as its contents, which for an empty panel is nothing.

  **The timetable and the right panel are not cards; the row above them is.** Neither has a radius or a fill — only hairlines, running edge to edge of their column. A card with a radius cannot be "100% height" without margins to float in, and margins are the opposite of what full height asks for, so both take the rail's treatment instead: a line and no box. The timetable's top rule begins exactly where the rail's vertical one ends. The consequence to remember: with no fill of its own, the timetable's sticky column headings take **`bg-ground`**, not `bg-surface`.

  The grid shows **five days, Monday to Friday** — `DAYS_SHOWN`, one constant that decides the column count, the now-line's span and the range label together. It slices the *front* of `weekDays()` rather than reordering, so Monday-first still agrees with the backend's `weekday`. Note the now-line's `gridColumn: 1 / DAYS_SHOWN + 2`: grid counts lines, not tracks, so five days plus the gutter is six columns and seven lines.

  **The timetable draws no bookings yet** — there is no fetch behind it, only the grid they will be positioned in. That is also what a real account sees before anyone books, so the shape is the same either way; `GET /appointments?from=&to=` is finished and waiting. Its hour window is a hard-coded 08:00–21:00, which becomes `openSpans()` from `lib/schedule.js` the moment the page reads the business's working hours.

  Three decisions in it worth keeping. **The calendar is its navigation** — there is no Today/Week/Month/Year switcher like the reference's, because three of those four segments would do nothing, and a segmented control where most segments are dead is worse than none. **The now-line is `accent`, not red**: red is the usual convention, but `danger` means something in this app and a permanent red rule across the busiest surface in the product spends that meaning on a clock. And **the sticky column headings are opaque `bg-surface`, never the selection tint** — the tint is an ink alpha, and the grid scrolls underneath them, so a translucent heading would show 15:00 through the word "THU"; the selected column is marked on the body below instead.

  Two things in the calendar are decisions rather than defaults, and both are departures from the reference it was drawn from. **The selected day is `accent`, not blue** — there is no brand hue left in this project, so the strongest contrast on the card does what the reference's blue did. And **weekends are not dimmed**: working hours are per-day on `/business`, so greying Saturday would tell a barbershop its busiest day is closed. The days to dim are the ones the schedule actually marks closed, once that is wired in. The grid is always **42 cells / six rows** (`monthGrid`), so the card does not change height four times a year and shift the page under the pointer of the very control you are stepping through.
...
**`src/lib/cities.js`, `appointments.js`, `businessOptions.js` and `schedule.js` are still untranslated** — about 150 Russian strings between them. They belong to screens that are still empty and are imported by nothing, so they are tree-shaken out of the build; translate them when those screens come back, not before.
