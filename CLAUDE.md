...
- `/business` — «Бизнес»: the owner-facing configuration route, and the first screen brought in line with the reference dashboard. **The layout is a 12-column grid, not a stack** — `BusinessProfile` owns it and takes a `trailing` prop for the last cell, so the page can hand it the placeholder card without a placeholder having to live inside a component called BusinessProfile. Row one splits 7/5 (profile | the placeholder), then «Услуги» and «График работы» run full width, because a five-column price list and a seven-column week genuinely need it — squeezing either into 58% is what would break. Five equal-width slabs is what the page was, and it is most of why it read as a settings panel.

  Two other things were fixed at the same time and are worth not undoing. **The profile field grid lost its cell borders** for a single hairline above each row — nine four-sided cells is the box-of-boxes treatment the reference never uses. And **service status is a tinted pill, not a switch**; off is muted rather than red, since a service the owner took down is not a fault.

  A `StatusCard` — four derived figures (services, average price, hours a week, profile completeness) in one hairline-split card — was built here and **removed on request**. Worth knowing before proposing it again: it is the reference's most characteristic move and it was the page's only 32px type, but the numbers a settings screen can derive about itself are ones the owner already typed in, so it told them nothing they did not know. Metrics belong on a screen that measures something that happened.
