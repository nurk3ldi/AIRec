...
### Public shell
- `PublicLayout.jsx` — **the only page transition in the app.** `/login` and `/signup` are deliberately visual twins, and swapped instantly they read as one page whose text glitched; a 260ms cross-fade makes them read as two states of one surface. **Opacity only, no slide** — the public pages are exactly one viewport tall (`min-height: calc(100vh - 64px)` under a 64px header), and a transform contributes to an ancestor's scrollable overflow even though it does not affect layout, so any downward travel puts a scrollbar on screen for the length of every transition. Built on `motion` (`LazyMotion` + `m` + `AnimatePresence`, `mode="wait"`), keyed on `pathname`, and it uses **`useOutlet()` rather than `<Outlet/>`** — the outlet element has to be captured into a keyed child or the leaving page renders the *arriving* route's content as it fades, which is the classic React Router + AnimatePresence bug. `initial={false}` so a hard load paints at once. `useReducedMotion()` collapses the whole thing to nothing rather than to a faster slide.
