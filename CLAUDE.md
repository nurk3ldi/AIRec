...
- `/profile/*` — the settings hub, wrapped by `components/ProfileLayout.jsx` (nav rail + section heading). `/profile` itself is a `getServerSideProps` redirect to `/profile/account`. **Only `/profile/account` is real** — it loads the user via `verifySession()`, saves through `PATCH /auth/me`, and owns the avatar upload/replace/delete controls and the sign-out button. `subscription`, `ai`, `business`, and `features` are `ComingSoon` stubs with no backend behind them. Adding a section means a page here plus an entry in `ProfileLayout`'s `SECTIONS` array — the rail is driven entirely by that array.
...
- **English** (by explicit request): the whole public flow (`LandingHeader.jsx`, `index.jsx`, `login.jsx`, `signup.jsx`, `forgot-password.jsx`, `reset-password.jsx`) and the whole `/profile/*` settings section, including its `aria-label`s and the `Profile` entry in `Header.jsx`'s `pageTitles` and the `Sidebar` profile tooltip.
