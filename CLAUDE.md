...
Two things there are load-bearing. The control is a **native `<select>` laid over the whole row at zero opacity** — we draw what you see, the browser runs what you operate. Same call as the login page's checkbox: for three options a select already has the keyboard handling, the typeahead, the form semantics, and the part no custom popup gets for free — the **system picker on a phone**, a wheel at the bottom of the screen rather than a menu floating inside a sheet. It covers the row, not just the value, so the label is tappable too (52px tall, full width). Being invisible, its focus ring would be too, so a sibling overlay paints hover and focus through `peer-hover` / `peer-focus-visible`; keep that overlay `pointer-events-none` or clicks stop reaching the select.
...
`ProfileDialog`'s `NARROW_SECTIONS` now holds `settings` beside `account` — both are a single narrow column, and the fixed 728×580 box left two rows stranded in the middle of it.
