...
- `/appointments` — «Записи»: **built, and this is the third attempt.** Two earlier designs were each taken down whole rather than edited, so this one inherited nothing from either — v1 (a 24-hour time grid) is in commit `1e0c045`, v2 (a full-width month calendar) was archived under `src/archive/` and that folder has since been deleted; both are in git history.
  ...
  **The timetable.** `components/appointments/Timetable.jsx` draws the week or the day, and the toolbar above it steers both.
  ...
  - **Closed hours are hatched, not filled** (`ClosedSpan`): a 135° 6/6 repeating gradient at 5% ink. A tint says the hour is some other kind of hour; hatching says it is struck out — and it survives a booking drawn on top of it, which a fill would not. A day off and a break are labelled with a dot and a word; the stretch before opening is not, because the empty grid already says it. `pointer-events-none`: the shading is information, not a wall.
  - **Booking cards are all the same grey** — `surface-chip` and a hairline. `BOOKING_COLORS` gave each booking of a day its own hue and was taken out: a week of coloured blocks is a week that looks like something is happening. What a card *shows* depends on how tall it is — the name at 28px, the service at 54, the span and price at 78, the status strip at 104 — and the order matters: the name first, then what they are here for, then the arithmetic.
  - **Status is the one thing that gets a colour, and only two of the four.** `completed` is `ok`, `no_show` is `danger`. `confirmed` is almost every booking there is, so it stays muted; `cancelled` already fades the whole card to 45%, which is stronger than a coloured word.
  ...
  **The toolbar** is «Записи» with a `StatusFilter` pill beside it on the left, and against the right edge `‹ ›`, a **«День» / «5 дней»** switcher and **«+ Добавить»**. The heading and the filter travel together because one names what is in the grid and the other decides it, where everything on the right decides which *days* are on screen. The arrows move the **page's** `selected`, the same state the calendar is bound to, so stepping a week here moves the month over there. How far an arrow steps comes from the view through `shiftDate`.
  ...
  **`BookingPopover` is a popover, not a modal**, anchored to the left of the «Добавить» button (`side="left"`, `align="center"`) with no scrim. The two are not interchangeable: a modal dims the page and takes it away, which is right for a form you went into on purpose and wrong for adding a booking, where the grid behind is the reason you know which day and hour you want. `collisionPadding` gives the top **80px** — the 68px header plus the usual 12 — because the panel is `z-[60]` and the header `z-40`, so nothing else stops it painting over the page title. It renders from `Timetable`, not from the page: Radix positions and traps focus against the element that opened it, so a trigger and its popover cannot live in different components.
  ...
  - **Time** — two `TimeField`s, start and end, **typed rather than picked**. `1430` becomes `14:30`; the two halves are padded in opposite directions (a lone `9` is nine o'clock, a lone `3` after the hour is half past), and out-of-range values are clamped rather than refused. A menu of every quarter hour was a list rather than a clock; a native `<input type="time">` drops the browser's own sheet; a hand-built pair of scrolling columns fixed the colours and kept the wrong idea. The end follows the service's length until it is moved by hand, and travels to the API as `duration_minutes`.
  - Client name, phone and note are ordinary fields.

  The panel is capped at 560px and scrolls inside itself, and the two blocks that grow with the business — the price list and the times — have ceilings of their own, so a long price list cannot push the client's name off the bottom. **Everything starts blank**, the date included: seeding the one service a new business happens to have was a value nobody entered, and a price that had not been agreed is the kind of default that gets saved by accident.
  ...
  **The calendar lives in the right panel** (`aside`, `xl:w-[calc(300px+2rem)]` — the calendar's own 300px plus its `p-4`, written as the sum so the two cannot drift). Beside the timetable it was a card sitting above thirteen hours of grid, taking height the one scrolling thing on the page had to give up. The panel is flush rather than rounded — a hairline and no box — for the same reason the timetable is: a card with a radius cannot be 100% of anything without margins to float in.
  ...
  Below `xl` the three regions stack and the page still does not scroll, so the calendar ends up below the fold. That is a small-screen layout still to be designed, not a scrollbar to add back.

  **What is still missing:** the right panel holds only the calendar — there is no booking-detail view, and therefore **no way to change a status, archive or delete from the UI** at all. `PATCH /appointments/{id}` and `DELETE` are finished and waiting.
...
| `surface-chip` | `#ffffff` | `#2a2a2a` | the thing that is *chosen* — a segment, the add button, today in the calendar, a booking card |
