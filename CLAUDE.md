...
- `/business` — «Бизнес»: the owner-facing configuration route, and the first screen brought in line with the reference dashboard. **The layout is a 12-column grid, not a stack** — `BusinessProfile` owns it and takes a `trailing` prop for the last cell, so the page can hand it the placeholder card without a placeholder having to live inside a component called BusinessProfile. Row one splits 7/5 (profile | «Состояние» + the placeholder stacked under it), then «Услуги» and «График работы» run full width, because a five-column price list and a seven-column week genuinely need it — squeezing either into 58% is what would break. Five equal-width slabs is what the page was, and it is most of why it read as a settings panel.

  Four things were fixed at the same time and are worth not undoing. **`StatusCard` gives the page its numbers**: it had none, and a screen whose largest text is a 20px heading has nothing for the eye to land on. Its four figures are derived from data the page already holds — no extra request — and they carry *no* coloured `+12%` delta, because nothing here has history and an invented percentage would be the one number on screen that lies. **The business name stays at 20px** precisely because those metrics are 32: 28 beside 32 is two adjacent steps side by side, which reads as a mistake. **The profile field grid lost its cell borders** for a single hairline above each row — nine four-sided cells is the box-of-boxes treatment the reference never uses. And **service status is a tinted pill, not a switch**; off is muted rather than red, since a service the owner took down is not a fault.

  «График работы» carries the page's one chart: `DayBar` draws each day's open stretches across a 24-hour track from `openSpans` in `lib/schedule.js`. It earns its place because a *break* is what splits a day in two, and seven rows of times can only say that by making you subtract. It is `aria-hidden` — the times sit right beside it, and announcing the row twice is worse than not announcing the picture.

  There used to be a second placeholder, «Настройка бизнеса», now dropped: the profile card *is* the business setup, so the empty box promised a screen sitting a few hundred pixels above it.

  **One page, no tabs** — it had an «О бизнесе» / «ИИ-ассистент» strip and the second was a placeholder, so the strip cost a row of chrome on every visit to offer a choice that did not exist; assistant settings will arrive as another card here, or as their own route if they outgrow the page. It's a real route rather than a profile-dialog section on purpose — a service list, a schedule and a staff roster don't fit a 520px dialog, and this is day-to-day work rather than account admin. The profile overlay is strictly "мен": account, security, subscription, app settings.
...
- **Only `/business` carries elevation so far.** `components/business/Card.jsx`
  has the resting shadow; the rest of the app was written under the old
  no-elevation rule and picks it up as screens are touched.
