...
**No labels**: words under five icons leave about 70pt each, which is where Russian starts truncating, and with five destinations the glyphs carry it. Each slot keeps an `aria-label`. The references mark the current tab by swapping an outline glyph for a filled one; **the free Hugeicons set ships strokes only**, so the accent plus a heavier stroke carries it instead — weight as well as colour, so it still reads without colour vision.
...
**The sections it opens are what behave differently.** `ProfileDialog` is a centred panel that fades and scales on a desktop, and on a phone a **full-bleed sheet rising from the bottom edge**, square-cornered, over the navigation rather than beside it — those are forms you went into on purpose, and a sideways tap out of one is not a way out worth offering. The × in its header is the way back. Which animation runs is a *value*, not a class, so it comes from `useMediaQuery('(min-width: 640px)')` in `lib/media.js` — reach for a `sm:` class first and come there only when a prop has to change.

Its layout comes from `design/profile_example.png`: identity centred — avatar with an edit badge, name, and **the email under the name rather than as a row**, since it identifies the account instead of leading anywhere and a row whose only job is to be read is a row you keep tapping by mistake. Then **named groups, each one card of several rows** divided by hairlines that start after the icon column, so the icons read as one strip down the group instead of being chopped by every line. A card is a *group*, not a row. Rows carry a value on the right where they have one. They open the same `ProfileDialog` the popup opens, so nothing below the entry point is duplicated.
