...
**`DashboardLayout` owns `isMenuOpen` and `dialogSection`**, and renders `ProfileMenu`/`ProfileDialog` itself. Both navigations open the same menu, and there must be exactly one of it — a second copy of the flag inside the bar would be a second menu the rail's outside-click handler could not close. `Sidebar` is now presentational.

`ProfileMenu` follows the same split: a sheet pinned above the bottom bar on a phone, the anchored 264px panel beside the rail from `sm` up, with `transformOrigin` at bottom centre so it still grows out of whatever opened it.
