...
export default function InboxPage() {
  ...
  return (
    ...
    >
      ...
      <div className="min-w-0 flex-[65] overflow-y-auto p-4 sm:p-6">
      ...
      <aside className="hidden min-w-0 flex-[35] flex-col p-4 lg:flex">
 ...
 * зазора между тремя карточками; высота идёт за шириной через `viewBox`.
 *
 * **`* 0.9` поверх доли.** Три в ряд без него заняли бы всю колонку;
 * умножение на 0.9 после деления — это и есть «на 10% меньше» для обеих
 * сторон сразу, поскольку высота у `Folder` идёт за шириной через
 * `aspect-ratio`, а не задаётся отдельно.
 */
    ...
    <div className="flex flex-wrap content-start gap-4 sm:gap-6">
      {rows.map((row, index) => (
        <Folder
          ...
          className="w-[calc((100%-2rem)/3*0.9)] sm:w-[calc((100%-3rem)/3*0.9)]"
        />
      ))}
    </div>
  )
...
const W = 400 // ширина
const H = 300 // высота
/* Ниже прежних 360: три в ряд уже шире, чем четыре были, и та же высота
   растягивала бы карточку в почти квадрат. Пропорции самой фигуры это не
   ломает — путь пересобирается из констант, и `R` остаётся прежним, поэтому
   углы остаются кругами того же радиуса, а не растягиваются вместе с телом.
   Язычок тоже не трогается: `TAB`, `SLOPE` и `DROP` заданы в единицах ширины
   и высоты по отдельности. */
