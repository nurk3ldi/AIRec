...
export default function InboxPage() {
  ...
  /**
   * Нажато ли «Все» у истории.
   *
   * **Это один ответ на один вопрос: показать всё.** Колонка отдаётся истории
   * целиком — секция дня уходит, и таблица занимает её место, — а вместе с ней
   * в таблицу возвращаются убранные разговоры. Разделять эти два действия
   * нечем: «всё» про день, который уже не показан, ничего не значит, а список
   * из половины истории под заголовком «Все чаты» противоречит сам себе.
   *
   * Отдельное состояние, а не параметр основного чтения: «Потоки» справа
   * показывают, кто пишет *сейчас*, и архивным там делать нечего.
   */
    ...
    >
      ...
      <div className="min-w-0 flex-[65] overflow-y-auto p-4 sm:p-6 lg:pr-3">
        {/* Собственного заголовка у страницы нет: на десктопе её называет
            ...
            по имени. */}
        {/* **Нажатое «Все» убирает день.** «Все чаты» тогда занимают колонку
            целиком, и это единственный честный ответ на такое нажатие: день и
            история — два ответа на один вопрос «кто писал», и показывать
            «всё» *под* выборкой одного дня значит говорить обе половины разом.
            Размонтируем, а не прячем: секция ничего не тикает и ничего не
            держит, а пустая строка заголовка под скрытым рядом была бы тем
            самым зазором, которого на экране быть не должно. */}
        {showAll ? null : (
          <Section
            title={t('inbox.today')}
        ...
        <Section
          ...
          className={showAll ? '' : 'mt-6 sm:mt-8'}
          actions={
            ...
            rows={
              all === null || chats === null
                ...
                : historyRows({
                    ...
                    // пустой из-за архива.
                    chats: showAll ? [...chats, ...(archived ?? [])] : chats,
 ...
 *
 * **У истории оно означает «показать всё»**: секция дня уходит, таблица
 * занимает колонку целиком и берёт в себя убранные разговоры. **Нажатое — это
 * уже другое слово, «Вернуть»**: кнопка одна, а работы у неё две, и вторую
 * надо называть. Вес состояние держал плохо — «Все» и «Все» полужирным
 * отличаются только тем, что их не с чем сравнить, — а «Вернуть» говорит и что
 * будет дальше, и что сейчас показано не то, что обычно.
 *
 * **У секции дня оно пока не нажимается, и это временно.** Показывать все
 * сегодняшние диалоги нечем: секция кормится записями (`GET /appointments`), а
 * диалоги — это `conversations`, и связи между ними в базе нет. Кнопка стоит
 * здесь, потому что место в раскладке решено; обработчик придёт вместе с
 * ответом на вопрос, что именно она разворачивает.
 */
