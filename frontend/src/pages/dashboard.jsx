 ...
 * берёт три пятых.
 *
 * **Три пятых, а не половина и не весь экран.** Половина оставляла разговорам
 * три-четыре строки, а целый экран означал бы, что ниже ничего нет — тогда как
...
export default function DashboardHomePage() {
  ...
  return (
    <div className={styles.page} aria-label={t('nav.dashboard')}>
      {/* **Доли, а не проценты.** `flex-[4]` и `flex-[6]` делят то, что
          осталось после зазора, поэтому 40/60 остаётся 40/60 при любом `gap` —
          `w-[40%]` и `w-[60%]` вместе с зазором дают больше ста и переносят
          ...
          чёрного на тёмной, то есть заливка, которая сама отделяет блок от
      ...
      <div className="flex w-full flex-col gap-4 p-4 sm:flex-row sm:gap-6 sm:p-6">
        <div
          className={`w-full min-w-0 rounded-2xl bg-surface-raised sm:flex-[4] ${TALL}`}
        />
        <div
          className={`w-full min-w-0 rounded-2xl bg-surface-raised sm:flex-[6] ${TALL}`}
        />
      </div>
    </div>
  )
