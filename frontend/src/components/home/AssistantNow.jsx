...
export default function AssistantNow({ chats, live, className = '' }) {
  ...
  return (
    ...
    >
      ...
      {/* **Портрет вместо значка, в левом верхнем углу.** Середина осталась
          содержанию: знак говорит, чья это карточка, и стоит там, откуда
          начинается чтение.
          ...
          `alt=""` — картинка декоративная: чья карточка, сказано словом
          «Ассистент» строкой ниже, и озвучивать её второй раз нечем.

          **Знак белый на прозрачном фоне, то есть виден только на тёмном.** На
          светлой теме карточка белая, и от робота останется тёмный экран с
          глазами, а корпус и скобки пропадут. Тому же правилу подчиняются
          логотипы проекта: `airec_logo.png` белый, `black_logo_icon.png` —
          чёрный вариант того же знака, и берётся тот, что подходит поверхности.
          Здесь второго варианта пока нет. */}
      <div className="flex h-[34%] shrink-0 items-start">
        <img src="/ai.png" alt="" className="h-full w-auto object-contain" />
      </div>
